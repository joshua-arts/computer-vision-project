...
def encode(hexcode, bar_width, bar_chunk_height_height):
  ...
  # Iterate through the hexcode and draw the corresponding bars.
  for i, c in enumerate(hexcode):
    ...
    if v % 2 == 0:
      start = (distance - bar_width, 0)
      end = (distance, bar_chunk_height * chunks)
    else:
      start = (distance - bar_width, height - (bar_chunk_height * chunks))
      end = (distance, height)
  ...
  width_pad = bar_width * 4
  encoded = cv2.copyMakeBorder(
    encoded,
    bar_chunk_height, bar_chunk_height,
    width_pad, width_pad,
    cv2.BORDER_CONSTANT,
    value = 255)
...
sample_hexcode = '0123456789abcdef'
random_hexcode = '1453ad8e82da85a3'
